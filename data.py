import os
import math
from typing import List, Optional, Literal, Union, NamedTuple
...
class TabularEncoderCMLP(nn.Module):
    def __init__(
        self,
        numeric_dim: int,
        categorical_cardinalities: List[int],
        cat_embed_dim: int = 16,
        num_hidden_dims: List[int] = [256, 256],  
        out_dim: int = 128,
        dropout: float = 0.1,
        reg_weights: Optional[dict] = None,
    ):
        ...
        self.cat_dim = len(categorical_cardinalities) * cat_embed_dim

        layers = []
        prev = numeric_dim
        for h in num_hidden_dims:
            layers += [
                nn.LayerNorm(prev),
                nn.Linear(prev, h),
                nn.SiLU(),
            ...
            prev = h
        self.num_mlp = nn.Sequential(*layers)  

        fuse_in_dim = num_hidden_dims[-1] + self.cat_dim
        self.out_proj = nn.Sequential(
            nn.LayerNorm(fuse_in_dim),
            nn.Linear(fuse_in_dim, out_dim),
        )

    def forward(
        self,
        numeric_tensor: torch.Tensor,
        categorical_idx: List[torch.Tensor],
    ):
        cat_vecs = []
        reg_total = numeric_tensor.new_zeros(())
        for emb, idx in zip(self.cat_embeddings, categorical_idx):
            ...
            cat_vecs.append(e)
            reg_total = reg_total + reg
        cat_emb = torch.cat(cat_vecs, dim=-1) if cat_vecs else None
        # cat_emb: [B, n_cat * cat_embed_dim]

        num_emb = self.num_mlp(numeric_tensor)
        # num_emb: [B, num_hidden_dims[-1]]

        if cat_emb is not None:
            fused = torch.cat([num_emb, cat_emb], dim=-1)
        else:
            fused = num_emb

        tab_embed = self.out_proj(fused)
        # tab_embed: [B, 128]

        return tab_embed, reg_total
...
class TabularEncoder_EmbedMLP(nn.Module):
    ...
    def forward(self, numeric_tensor: torch.Tensor, categorical_idx: List[torch.Tensor]):
        ...
        loss = numeric_tensor.new_zeros(())
        return tab_embed, loss
