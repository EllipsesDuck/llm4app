...
class FlashDecoderSFT(nn.Module):
    ...
    def forward(self, target_ids, user_static, short_term, long_term=None, **kwargs):
    ...
    def forward_with_hidden(self, target_ids, user_static, short_term, long_term=None, **kwargs):
        user_static, short_term, long_term = self._adapt_ctx(user_static, short_term, long_term)
        out = self.decoder(
            target_ids=target_ids,
            user_static=user_static,
    ...
    def generate_with_logprobs(
        ...
        self.eval()
        user_static, short_term, long_term = self._adapt_ctx(user_static, short_term, long_term)
        return self.decoder.generate_with_logprobs(
            input_ids=input_ids,
            user_static=user_static,
            short_term=short_term,
            long_term=long_term,
...
def eval_split(
    ...
    for batch in tqdm(dl, desc=name, leave=False):
        ...
        if gen_ids.size(1) < (1 + code_len):
            ...
            gen_ids = torch.cat(
                [gen_ids, torch.full((bs, pad_more), pad_id, device=device, dtype=torch.long)],
                dim=1
            )
...
def train_one_epoch_sft(
    ...
    pbar = tqdm(dl_tr, desc="train_sft", leave=False)
    for batch in pbar:
        global_step += 1
        target_ids = batch["target_ids"].to(device, non_blocking=True)
        user_static = batch["user_static"].to(device, non_blocking=True)
        ...
        if writer is not None and (global_step % max(1, log_every) == 0):
            writer.add_scalar("train_sft/loss_step", float(loss.item()), global_step)
            writer.add_scalar("train_sft/lm_loss_step", float(loss_lm.item()), global_step)
            writer.add_scalar("train_sft/cls_loss_step", float(loss_cls.item()), global_step)
            writer.add_scalar("train_sft/lr", opt.param_groups[0]["lr"], global_step)
    ...
    for batch in pbar:
        ...
        with torch.no_grad():
            ...
            if gbpo.fixed_prefix_len <= 0:
                start_ids = torch.full((B, 1), int(bos_id), device=device, dtype=torch.long)
            else:
                start_ids = target_ids[:, :gbpo.fixed_prefix_len]

            gen_ids, _, _ = model.generate_with_logprobs(
                input_ids=start_ids,
                user_static=user_static,
                short_term=short_term,
                long_term=None,
        ...
        batch_rl = {
            ...
            "rewards": rewards,  # (B,)
            "step": global_step,
            "total_steps": max(1, len(dl_tr)),
            "max_new_tokens": int(max_new_tokens),
            "temperature": float(temperature),
            "eos_id": int(eos_id),
        }

        stats = gbpo.train_step(batch_rl, opt, use_rl=True)

        if (sync_old_every > 0) and (global_step % sync_old_every == 0):
            gbpo.sync_old_policy()

        bs = target_ids.size(0)
        total += stats["loss"] * bs
        total_ce += stats["loss_ce"] * bs
        ...
        if writer is not None and (global_step % max(1, log_every) == 0):
            writer.add_scalar("train_gbpo/loss_step", stats["loss"], global_step)
...
def main():
    ...
    ap.add_argument("--tab_root", type=str, default=r"E:\NUS\data\perdata\train_text_all_samples\tab_embed_v1_mlp")
    ap.add_argument("--ts_root", type=str, default=r"E:\NUS\data\perdata\train_text_all_samples\ts_emb_tsf_v1")
    ...
    ap.add_argument("--alpha_cls", type=float, default=0.0, help="cls loss weight (SFT only); 0 means disable")
    ...
    # ---- RL / GBPO ----
    ap.add_argument("--use_rl", action="store_true", help="enable GBPO RL training")
    ap.add_argument("--lambda_rl", type=float, default=0.1)
    ap.add_argument("--clip_eps", type=float, default=0.2)
    ap.add_argument("--gbpo_level", type=str, default="sequence", choices=["sequence", "token"])
    ap.add_argument("--gbpo_use_clip", action="store_true")

    ap.add_argument("--fixed_prefix_len", type=int, default=0)
    ap.add_argument("--use_auto_prefix", action="store_true")
    ap.add_argument("--use_hybrid_prefix", action="store_true")
    ap.add_argument("--auto_prefix_mode", type=str, default="linear", choices=["linear", "cosine", "exp"])

    ap.add_argument("--rl_max_new_tokens", type=int, default=3)
    ...
    vocab_size = (max_code + args.code_offset) + 3
    ...
    if args.use_rl:
        gbpo = GBPOTrainer(
            ...
            pad_id=args.pad_id,
            bos_id=args.bos_id,
            gbpo_level=args.gbpo_level,
            gbpo_use_clip=args.gbpo_use_clip,
            gbpo_mask_bos=True,
            device=torch.device(device),
            semantic_processor=None,
            fixed_prefix_len=args.fixed_prefix_len,
            use_auto_prefix=args.use_auto_prefix,
            use_hybrid_prefix=args.use_hybrid_prefix,
            auto_prefix_mode=args.auto_prefix_mode,
        )
    ...
    for ep in tqdm(range(1, args.epochs + 1), desc="Epochs", leave=True):
        if not args.use_rl:
            tr_stats, global_step = train_one_epoch_sft(
            ...
            tr_stats, global_step = train_one_epoch_gbpo(
                model=model,
                gbpo=gbpo,
                dl_tr=dl_tr,
                opt=opt,
