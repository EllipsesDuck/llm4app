...
def pivot_procedureevent(df, event_list):
    ...
    for event in event_list: 
        df1[event] = np.nan
        df1.loc[(df['label']==event), event] = df['value'].astype(float)  
    df_out = df1.dropna(axis=0, how='all', subset=event_list)
    return df_out 
...
def get_single_chest_xray_embeddings(img):
    ...
    #model_weights_name = "densenet121-res224-all" 
    #model_weights_name = "densenet121-res224-rsna" 
    #model_weights_name = "densenet121-res224-nih" 
    #model_weights_name = "densenet121-res224-pc") 
    model_weights_name = "densenet121-res224-chex" 
    #model_weights_name = "densenet121-res224-mimic_nb" 
    #model_weights_name = "densenet121-res224-mimic_ch" 
    #model_weights_name = "resnet50-res512-all"
    ...
    with tqdm(total = nImgs) as pbar:
        for idx, img in enumerate(imgs):
            ...
            with torch.no_grad():
                ...
                feats = F.adaptive_avg_pool2d(feats, (1, 1))
                densefeatures = feats.cpu().detach().numpy().reshape(-1)
                densefeature_embeddings.append(densefeatures)
                
                preds = model(img).cpu()
                predictions = preds[0].detach().numpy()
                prediction_embeddings.append(predictions) 
